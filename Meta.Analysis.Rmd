...
do.multiple.regressions <- function(
  outcome, preferred.scale = FALSE, moderator.multiple.list,
  ...
  degree.1 = TRUE, degree.2 = TRUE, non.interaction = TRUE, interaction = FALSE
...
# Linear
    if (degree.1){
      ...
      if (print.regplot & length(moderator.combination) == 2){
        ...
        if (non.interaction){
          model.res.list <- list("", "", "", "")
          names(model.res.list) <- c("min", "mean", "max", "data")
        }
        
        if (interaction){
          model.res.list.interact <- list("", "", "", "")
          names(model.res.list.interact) <- c("min", "mean", "max", "data")
        }
...
```{r}
results.meta.stress.dass <- print.meta.results("Stress", preferred.scale = "DASS", return.data = "results.meta", regression = F)
```
...
server <- function(input, output) {
  ...
  output$plot1 <- renderPlot({
    forest(results.meta.stress.dass)
  })
}
