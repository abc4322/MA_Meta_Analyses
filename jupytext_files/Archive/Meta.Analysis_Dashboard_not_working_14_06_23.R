...
#   regression = F, print.forest = T, print.funnel = F, print.influence.metafor = F, print.baujat = F, split.subgroups = F, print.forest.sub.single = F, print.meta.results = F,
...
# includes...
  ...
  # print plots or data
  print.forest = F, print.funnel = F, funnel.label = T, print.meta.results = F,
  print.descriptive = F, print.influence.metafor = F, print.influence.dmetar = F,  print.baujat = F, report = F,
...
# Print Forest plot or descriptive or meta-results
    ...
    if(results.meta$k >= 3){
      results.influence.metafor <- get.results.metafor(meta.df.list[[1]])
      
      if (print.influence.metafor){
        par(mar = c(5, 6, 4, 2))  # to set plot parameters?
        plot(influence(results.influence.metafor), cex = 0.8, las = 1)
      }
      
      if (print.influence.dmetar){
        results.meta.inf <- InfluenceAnalysis(results.meta, random = TRUE)
        plot(results.meta.inf, "influence")
      }
      
      if (print.baujat){
        # results.influence.meta <- InfluenceAnalysis(results.meta, random = T)
        # plot(results.influence.meta, "baujat")  # plot by dmetar
        baujat(results.influence.metafor)  # plot by metafor
      }
      
    } else if ((print.influence.metafor | print.baujat) & results.meta$k < 3){
...
# Return Data
  if (moderator == F & return.data != F){
    if (return.data == "meta.df.list"){
      ...
      print('set return.data to "meta.df.list", "funnel.asym.p.egger", "funnel.asym.p.egger.rank", "descriptive", "regression.all", "results.meta", "results.metafor", "hedges.g", "regression.results.linear", or "regression.results.poly"')
    }
...
print.meta.results <- function(
  outcome, preferred.scale = F, print.meta.results = T, meta.df.list = F, return.data = F,
  
  # arguments for forest and funnel plot
  basic = T, 
  print.forest = T, print.funnel = T, funnel.label = T, print.influence.metafor = T, print.influence.dmetar = F,
  print.baujat = T, split.subgroups = T, print.forest.sub.single = F,
  ...
  regression.degree.1 = T, regression.degree.2 = T, filter.regression.vec = F, print.regression.df = F,
...
# Forest and Funnel Plot
  if (basic){
    return.data.var <- meta.analyze(
      ...
      print.meta.results = print.meta.results, print.influence.metafor = print.influence.metafor,
      print.influence.dmetar = print.influence.dmetar,  return.data = return.data,
  ...
  if (regression){
    ...
    for (moderator in moderator.vec){
      ...
      regression.df <- meta.analyze(
        ...
        split.subgroups = F, print.influence.metafor = F, without.mean.r = without.mean.r
      )
    ...
    if (results.meta$k > 0){
      ...
      k <- append(
        ...
        length(unique(
          print.meta.results(
            outcome, preferred.scale = preferred.scale,
            regression = F, print.forest = F, print.funnel = F, print.influence.metafor = F, print.baujat = F, split.subgroups = F, print.forest.sub.single = F, print.meta.results = F,
...
# regression.multiple.no.outliers.tab.name <- "M. Regressions (w.o.)"egg
      ...
      tabPanel(
        ...
        fluidRow(
          ...
          box(
            ...
            background = if(funnel.asym.p.rank <= 0.05 | funnel.asym.p.egger <= 0.05){sig.0.05.color}else{NULL},
            footer = paste(
              "P-value of Egger's regression and rank correlation test for funnel plot asymmetry:",
              round(funnel.asym.p.egger, digits = 4), round(funnel.asym.p.rank, digits = 4)
            ),
            width = 4,
            class = "well",
...
# Main Results comparison with/without outliers
      tabPanel(
        ...
        fluidRow(
          ...
          box(
            title = "Influence Plots (metafor)",
            width = 4,
            class = "well",
            plotOutput(paste("influence.metafor", outcome.alias, ".forest", sep = "")),
            align="center",
            collapsible = TRUE,
            collapsed = TRUE
          ),
          box(
            title = "Influence Plots (dmetar)",
            width = 4,
            class = "well",
            plotOutput(paste("influence.dmetar", outcome.alias, ".forest", sep = "")),
            align="center",
            collapsible = TRUE,
            collapsed = TRUE
          )
        ...
        fluidRow(
          ...
          box(
            ...
            footer = paste(
              "P-value of Egger's regression and rank correlation test for funnel plot asymmetry:",
              round(funnel.asym.p.egger.n.o., digits = 4), round(funnel.asym.p.rank.n.o., digits = 4)
            ),
            align="right",
            collapsible = TRUE
          )
...
return.outcome.output <- function(output, outcome.vec, preferred.scale = FALSE){
  ...
  # Main Results
    ...
    output[[paste("subgroup.delivery.", outcome.alias, sep = "")]] <- renderPlot(
      {
        print.meta.results(
          outcome, preferred.scale = preferred.scale, meta.df.list = meta.df.list,
          regression = F, print.forest = F, print.funnel = F, print.influence.metafor = F, print.baujat = F, split.subgroups = T, print.forest.sub.single = "delivery.mode", print.meta.results = F  # ,
          # filter.forest..funnel.vec = if(length(outliers) == 0){FALSE}else{-outliers}
        )
      },
    ...
    output[[paste("subgroup.type.", outcome.alias, sep = "")]] <- renderPlot(
      {
        print.meta.results(
          outcome, preferred.scale = preferred.scale, meta.df.list = meta.df.list,
          regression = F, print.forest = F, print.funnel = F, print.influence.metafor = F, print.baujat = F, split.subgroups = T, print.forest.sub.single = "meditation.type", print.meta.results = F  # ,
          # filter.forest..funnel.vec = if(length(outliers) == 0){FALSE}else{-outliers}
        )
      },
      height = 600
    ...
    output[[paste("funnel.", outcome.alias, sep = "")]] <- renderPlot({
      print.meta.results(
        outcome, preferred.scale = preferred.scale, meta.df.list = meta.df.list,
        regression = F, print.forest = F, print.funnel = T, print.influence.metafor = F, print.baujat = F, split.subgroups = F, print.forest.sub.single = F, print.meta.results = F,
        funnel.label = F  # ,
        # filter.forest..funnel.vec = if(length(outliers) == 0){FALSE}else{-outliers}
      )
    })
  ...
  # Main Results comparison with/without outliers
    output[[paste("baujat.", outcome.alias, ".forest.comp", sep = "")]] <- renderPlot({  # n.o. = no outliers (included)
      print.meta.results(
        outcome, preferred.scale = preferred.scale, meta.df.list = meta.df.list,
        regression = F, print.forest = F, print.funnel = F, print.influence.metafor = F, print.baujat = T, split.subgroups = F, print.forest.sub.single = F, print.meta.results = F
      )
    })
    
    output[[paste("influence.metafor", outcome.alias, ".forest", sep = "")]] <- renderPlot({
      print.meta.results(
        outcome, preferred.scale = preferred.scale, meta.df.list = meta.df.list,
        regression = F, print.forest = F, print.funnel = F, print.influence.metafor = T, print.baujat = F, split.subgroups = F, print.forest.sub.single = F, print.meta.results = F
      )
    })
    
    output[[paste("influence.dmetar", outcome.alias, ".forest", sep = "")]] <- renderPlot({
      print.meta.results(
        outcome, preferred.scale = preferred.scale, meta.df.list = meta.df.list,
        regression = F, print.forest = F, print.funnel = F, print.influence.metafor = F, print.influence.dmetar = T, print.baujat = F, split.subgroups = F, print.forest.sub.single = F, print.meta.results = F
      )
    })
...
#     print.forest = F, print.funnel = F, print.meta.results = F, split.subgroups = F, print.influence.metafor = F, regression = F, print.baujat = F
#   )
...
# %% [markdown]
# ## Test (Influence Analysis and Thresholds)
...
# %%
test.metafor <- print.meta.results(
...
# %%
names(influence(model = test.metafor))

# %%
data.frame(influence(test.metafor)$inf)

# %%
test.inf <- influence(test.metafor)

# %%
test.df <- data.frame(test.inf$inf)
test.df

# %%
cook.d.thresh <- median(test.df$cook.d) + 6 * IQR(test.df$cook.d)
cook.d.thresh

# %%
DFFITS.thresh <- 3 * sqrt(test.inf$p / (test.inf$k / test.inf$p))
DFFITS.thresh

# %%
hat.tresh <- 3 * (test.inf$p / test.inf$k)
hat.tresh

# %%
plot(influence(test.metafor))
...
# %%
print.meta.results("Anxiety (trait)", filter.forest..funnel.vec = -c(6), regression = F, split.subgroups = F, print.influence.metafor = F)
...
# %%
print.meta.results("Depression (trait)", filter.forest..funnel.vec = -c(1, 2, 7), regression = F, split.subgroups = F, print.influence.metafor = F)
...
print.meta.results(
  ...
  regression = T, split.subgroups = T, print.influence.metafor = F, print.baujat.regression = F, print.gosh.regression = F
)
...
print.meta.results(
  ...
  regression = T, split.subgroups = F, print.influence.metafor = F, print.baujat.regression = F, print.gosh.regression = T
)
...
# %%
print.meta.results("Well-being or quality of life", filter.forest..funnel.vec = -c(1), regression = F, split.subgroups = F, print.influence.metafor = F)
...
# %%
# exclude outlier
print.meta.results("Mindfulness (trait)", filter.forest..funnel.vec = -c(9), regression = F, split.subgroups = F, print.influence.metafor = F)
...
# %% [markdown]
# ## Outlier and influential analysis with {dmetar}
...
test.meta.results <- update.meta(
  print.meta.results(
    "Stress", preferred.scale = "DASS",
    regression = F, print.forest = T, print.funnel = F, print.influence.metafor = T, print.baujat = F, split.subgroups = F, print.forest.sub.single = F, print.meta.results = F,
    return.data = "results.meta"
  ),
  prediction = TRUE
)
...
# find.outliers outliers by function
test.meta.results.n.o. <- find.outliers(test.meta.results)  # function of dmetar
forest(test.meta.results.n.o.)

# %%
test.meta.results.n.o.

# %%
test.meta.results.inf <- InfluenceAnalysis(test.meta.results, random = TRUE)

# %%
plot(test.meta.results.inf)
...
#   # print plots or data
#   print.forest = FALSE, print.funnel = FALSE, print.meta.results = FALSE,
#   print.descriptive = FALSE, print.influence.metafor = FALSE, report = FALSE,
...
#     if(print.influence.metafor & results.meta$k >= 3){
...
#     } else if (print.influence.metafor & results.meta$k < 3){
