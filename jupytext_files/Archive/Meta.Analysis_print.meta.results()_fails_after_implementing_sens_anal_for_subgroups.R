...
# %% [markdown] cell_style="center" hidden=true
# ## Get all present outcomes names

# %% hidden=true
intervention.comparisons.df.list

# %% cell_style="center" code_folding=[] hidden=true
#
present.outcomes <- c()
...
# includes...
  ...
  print.forest = F, forest.add.fix.eff.mod = T, forest.add.power = F, print.funnel = F, funnel.label = T, funnel.label.out.only = T, print.meta.results = F,
    # post hoc power often gets misinterpreted and delivers limited information --> should not be used or treated with caution
    # --> Wang (2010). Retrospective Statistical Power: Fallacies and Recommendations. Elsevier Inc. doi:10.1053/j.nainr.2009.12.012
  print.descriptive = F, print.influence = F, print.baujat = F, report = F,
  
  # regression and subgroup analysis 
  moderator = F, split.subgroups = T, subgroups = c("delivery.mode", "meditation.type"), print.forest.sub.single = F, subgroup.method = "fixed",
  without.mean.r = F,
      ...
      if (results.meta$k >= 4){
        ...
        for (subgroup in subgroups){
          ...
          if (subgroup.method == "fixed"){  # corresponds to Schwarzer et al. (2015). Meta-Analysis with R. doi: 10.1007/978-3-319-21416-0, pp. 41 - 45 & 89 - 91
            ...
            # get separete results for both levels of subgroups (random effect model)
            results.meta.sub.asy <- metacont(
              n.e = n.int, mean.e = mean.int, sd.e = sd.int,
              n.c = n.control, mean.c = mean.control, sd.c = sd.control,
            ...
            TE.del <- c(results.meta.sub.asy$TE.random, results.meta.sub.sy$TE.random)
            
            # corresponding standard error
            seTE.del <- c(results.meta.sub.asy$seTE.random, results.meta.sub.sy$seTE.random)

            # meta-analysis of subgroup estimates (fixed effect model)
            results.meta.sub <- metagen(
              TE.del, seTE.del, sm="SMD",
              studlab = c("asynchronous guiding", "synchronous guiding"),
              random = F, common = T
            )
...
# Return Data
  if (moderator == F & return.data != F){
    if (return.data == "meta.df.list"){
      ...
      # results.meta for print.forest.sub.single != F are returned above
...
print.meta.results <- function(
  ...
  basic = T, 
  print.forest = T, forest.add.fix.eff.mod = T, forest.add.power = F, print.funnel = T, funnel.label = T, funnel.label.out.only = T, print.influence = T, print.baujat = T,
  split.subgroups = T, print.forest.sub.single = F, subgroup.method = "fixed",
...
# Forest and Funnel Plot
  if (basic){
    return.data.var <- meta.analyze(
      ...
      subgroup.method = subgroup.method, exclude.high.rob.vec = exclude.high.rob.vec, sort.by = "hedges.g",
      without.mean.r = without.mean.r  # print.descriptive = T
    )
  }
...
# %%
get.sens.anal.vec <- function(res, model, model.meta = NA){ 
  ...
  if ("rma.uni" %in%  class(res)){  # for {metafor} results
    ...
    if (model == "rand.fix"){
      ...
      if (nrow(res$b) == 2){  # linear model --> 2 model coefficients
        sens.anal <- c(
      ...
      } else if (nrow(res$b) == 3){  # squaerd model --> 3 model coefficients
        sens.anal <- c(
          res$k, n.total, n.int, n.cont,
          res$b["intrcpt",1], res$ci.lb[1], res$ci.ub[1], res$se[1], res$zval[1], pval[1],
          res$b[2,1], res$ci.lb[2], res$ci.ub[2], res$se[2], res$zval[2], pval[2],
    ...
    if (model == "rand.fix"){
      ...
      print("there is no regression calculated by {meta} on its own (only a wrapper function working with {metafor})")
    }
  } else {
...
get.sens.anal.df <- function(
  ...
  if (model == "rand.fix"){
    ...
    if (restuls.meta.o.$k > 0){
            ...
            for (lib in c("meta", "metafor")){
              for (model.used in c("random", "common")){
                ...
                sens.anal.vec.df <- t(data.frame(
                  get.sens.anal.vec(res, model, model.used),
                  row.names = cnames
                ))
      ...
      sens.anal.df[, 6:10] <- round(sens.anal.df[, 6:10], 4)
          ...
          data_color(
           ...
           tab_footnote(
            footnote = paste(
              ...
              "outliers and influential cases included - yes = 1, no = 0;",
...
# Regression
    ...
    for (moderator in moderator.vec){
      ...
      cnames <- c(
        "Number of included studies", "Number of obeservations", "... in experimental group", "... in control group",
        ...
        "I^2 [%]", "Q-value", "p-value (testing between-study heterogeneity)", "p-value for omnibus-test of moderators"        
      )
      ...
      ### coefficients
      for (coefficient in c("intercept", "1st moderator coefficient", "2nd moderator coefficient")){
        lo.lim <- ifelse(coefficient == "intercept", 6, up.lim + 1)
        up.lim <- lo.lim + 5
          ...
          data_color(
           ...
           tab_footnote(
            footnote = ifelse(
              ...
              paste(
                "Meaning of digits in decision codes (same digit order):",
                "preferred Scale - DASS = 1, PSS = 0; outliers and influential cases included - yes = 1, no = 0;",
                "'sub-model' used in mixed-effects model - random-effects = 1, fixed-effects = 0; mean range values - included = 1, excluded = 0;",
                "degree of model - linear = 1, squared = 0"
              ),
              ""
            )
          ) |>
...
# %% [markdown]
# ### Test - getting different results of subgroup analysis

# %%
test.res.sub.fix <- print.meta.results(
  "Stress", preferred.scale = "DASS",
  regression = F, print.forest = F, print.funnel = F, print.influence = F, print.baujat = F, split.subgroups = T, print.forest.sub.single = "delivery.mode",
  subgroup.method = "fixed", print.meta.results = F,
  return.data = "results.meta"
)
...
print.meta.results(
  "Stress", preferred.scale = "DASS",
  regression = F, print.forest = F, print.funnel = F, print.influence = F, print.baujat = F, split.subgroups = T, print.forest.sub.single = "delivery.mode",
  subgroup.method = "random.separate.tau2", print.meta.results = F,
  return.data = "results.meta"
)
...
print.meta.results(
  "Stress", preferred.scale = "DASS",
  regression = F, print.forest = F, print.funnel = F, print.influence = F, print.baujat = F, split.subgroups = T, print.forest.sub.single = "delivery.mode",
  subgroup.method = "random.common.tau2", print.meta.results = F,
  return.data = "results.meta"
)
...
print.meta.results(
  "Stress", preferred.scale = "DASS",
  regression = F, print.forest = F, print.funnel = F, print.influence = F, print.baujat = F, split.subgroups = F, print.meta.results = F,
  return.data = "meta.df.list"
)
...
# %%
test.metafor.sub <- print.meta.results(
  "Stress", preferred.scale = "DASS",
  basic = F, moderator.vec = c("delivery.mode"), print.regplot = F, print.baujat.regression = F, print.regression.results = F, regression.degree.1 = T, regression.degree.2 = F,
...
# %%
get.sens.anal.df("Anxiety (trait)", "rand.fix", save.as = "html")

# %%
get.sens.anal.df("Stress", "mixed", moderator.vec = c("programs.duration"), save.as = "html")
...
regression.single.with.outliers.tab.name <- "Regressions (with outliers)"
regression.single.comp.tab.name <- "Regressions (comparison with/without outliers; with mean ranges)"
regression.single.comp.tab.name.no.mean.r <- "Regressions (comparison with/without outliers; without mean ranges)"
...
# %% [markdown]
# ### Subgroup analysis with random effects model and separate tau2 
...
# %%
# load version 4.0-2 of meta package
## unload meta and packages requiring meta
unloadNamespace("dmetar")
unloadNamespace("netmeta")
unloadNamespace("meta")

## name path where old version of meta ist installed
meta_lib_path_old_version <- paste(Sys.getenv("USERPROFILE"), "R_old_packages", sep = "\\")

## load package
library(package = "meta", lib.loc = meta_lib_path_old_version)
...
test.meta.sub.del.res <- metagen(
  test.TE.del, test.seTE.del, sm="SMD",
  random=FALSE, common = T
)
test.meta.sub.del.res

# %% [markdown]
# ### Subgroup analysis with random effects model and common tau2 (and installing old package version of {meta})

# %%
# # install.packages("remotes")  # for install old package version
# library("remotes")

# # unload met and packages requiring meta to be able to install older version
# unloadNamespace("dmetar")
# unloadNamespace("netmeta")
# unloadNamespace("meta")

# meta_lib_path_old_version <- paste(Sys.getenv("USERPROFILE"), "R_old_packages", sep = "\\")  # filder "R_old_package_versions" in user's directory

# # install with install.packages()  # worked on R console
# packageurl <- "https://cran.r-project.org/src/contrib/Archive/meta/meta_4.0-2.tar.gz"
# install.packages(packageurl, repos=NULL, type="source", lib = meta_lib_path_old_version)

# # install with {remotes}  # was not successful
# remotes::install_version("meta", "4.0-2", lib = meta_lib_path_old_version)
...
mc3s.p <- metacont(
  ...
  data = test.meta.df.list[[1]], sm = "SMD",
  studlab = study.id,
  byvar=delivery.mode, print.byvar=FALSE,
...
# %% [markdown]
# ### Random Effects Model with Common Estimate of tau2

# %%
test.res.check.short <- metacont(
  n.e = n.int, mean.e = mean.int, sd.e = sd.int,
  n.c = n.control, mean.c = mean.control, sd.c = sd.control,
  data = test.meta.df.list[[1]], sm = "SMD",
  studlab = study.id
)
test.res.check.short

# %%
update.meta(test.res.check.short, subgroup = delivery.mode, tau.common=TRUE, common=FALSE, random = TRUE)
...
mc3s.c.new <- metacont(
  ...
  data = test.meta.df.list[[1]], sm = "SMD",
  studlab = study.id,
  subgroup=delivery.mode, print.subgroup=FALSE,
  tau.common=TRUE, fixed=FALSE)
print(summary(mc3s.c.new))
